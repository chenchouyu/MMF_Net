...
class MF(nn.Module):
    def __init__(self):
        ...
        self.Up1 = nn.Sequential(
            nn.Upsample(scale_factor=2),
            DoubleConv(inputFeature=64, outputFeature=16)
        )

        self.Up2 = nn.Sequential(
            nn.Upsample(scale_factor=4),
            DoubleConv(inputFeature=256, outputFeature=8)
        )

        self.Up3 = nn.Sequential(
            nn.Upsample(scale_factor=8),
            DoubleConv(inputFeature=512, outputFeature=4)
        )

        self.Conv1 = DoubleConv(15, 1)
        self.Conv2 = DoubleConv(15, 1)
        self.Conv3 = DoubleConv(15, 1)
        self.Conv4 = DoubleConv(15, 1)
...
class Net(nn.Module):
    def __init__(self, in_ch=4):
        ...
        n = 32
        filters = [n, n * 2, n * 8, n * 16, n * 32]

        self.maxPool = nn.MaxPool2d(kernel_size=2, stride=2)

        res = res2net50(pretrained=True)
        ...
        self.encoder3 = res.layer1  # 256
        self.encoder4 = res.layer2  # 512

        self.Middle = nn.Sequential(
            res.layer3,
            Transformer(filters[-1], size=3)
        )
    ...
    def forward(self, x):
        ...
        c1 = self.encoder1(x)  # 32
        c2 = self.encoder2(self.maxPool(c1))  # 64
        c3 = self.encoder3(self.maxPool(c2))  # 256
        c4 = self.encoder4(c3)  # 512

        y = self.Middle(c4)  # 1024

        f4 = self.decoder4(y, c4)  # 512
        f3 = self.decoder3(f4, c3)  # 256
        f2 = self.decoder2(f3, c2)  # 64
        f1 = self.decoder1(f2, c1)  # 32
